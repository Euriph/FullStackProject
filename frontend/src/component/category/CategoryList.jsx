          ...
          filterData(CategoryStateApi).map((data) =>
            <tr key={data.id} className="table-todo">
              ...
              <td style={{ textDecoration: data.todoComplete === 'COMPLETE' ? 'line-through red' : 'none' }}>
                <input
                  type="checkbox"
                  checked={data.todoComplete === 'COMPLETE'} // Checkbox should be checked if the status is 'COMPLETE'
                  onChange={() => toggleCheckbox(data.id, data.todoComplete !== 'COMPLETE')} // Toggle between 'COMPLETE' and 'INCOMPLETE'
                />
